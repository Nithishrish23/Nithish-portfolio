...
const work = [
  { index: '01', title: 'Salesforce AI Copilot', category: 'AI / Enterprise Automation', description: 'A permission-aware AI system for Salesforce that combines RAG, MCP tool orchestration and conversational workflows with approval, validation and auditability before critical actions.', stack: ['FastAPI', 'React', 'RAG', 'MCP', 'Salesforce', 'OpenAI', 'Claude'], repo: 'https://github.com/Nithishrish23/Salesforce-ai' },
  { index: '02', title: 'Multi-Agent Finance Platform', category: 'AI Agents / Finance', description: 'A production-oriented multi-agent platform with provider-agnostic LLM routing, workflow orchestration, memory, finance analysis, notifications and a fail-closed risk layer.', stack: ['Next.js', 'FastAPI', 'PostgreSQL', 'Redis', 'Celery', 'LLM Gateway'], repo: 'https://github.com/Nithishrish23/agent-earning' },
  { index: '03', title: 'Full-Stack Commerce Platform', category: 'Product Engineering', description: 'B2B/B2C e-commerce engineering across customer, seller and admin workflows, with Python APIs, PostgreSQL, authentication, authorization and payment integration.', stack: ['React', 'Vite', 'Flask', 'PostgreSQL', 'JWT', 'Stripe'], repo: 'https://github.com/Nithishrish23' },
  { index: '04', title: 'KYC Document Intelligence', category: 'Computer Vision', description: 'Document identification and verification workflows using computer vision for PAN, Aadhaar, address and signature documents, including Aadhaar masking.', stack: ['Python', 'YOLO', 'OpenCV', 'PIL', 'Computer Vision'], repo: 'https://github.com/Nithishrish23' },
];
  ...
  return (
    <div className="site">
      ...
      <header className="nav-wrap"><nav className="nav container">
        <button className="brand" onClick={() => go('home')} aria-label="Back to home"><img className="brand-photo" src={PROFILE_IMAGE} alt="Nithish Kumar"/><span><strong>Nithish Kumar</strong><small>AI ENGINEER · FULL STACK</small></span></button>
        <div className={`nav-links ${menuOpen ? 'open' : ''}`}>{['about', 'work', 'experience', 'contact'].map((item) => <button key={item} onClick={() => go(item)}>{item}</button>)}<a href="https://github.com/Nithishrish23" target="_blank" rel="noreferrer">GitHub <ArrowUpRight size={15}/></a></div>
      ...
      <main>
        <section id="home" className="hero container">
          <div className="hero-copy reveal"><div className="hero-line"><span /> BUILDING INTELLIGENT SYSTEMS</div><h1>AI that feels <em>engineered.</em><br/>Products that feel <em>alive.</em></h1><p className="hero-text">I'm Nithish Kumar — a Software Engineer II focused on LLM applications, RAG, AI agents, enterprise automation and full-stack products that turn complex ideas into dependable software.</p><div className="hero-actions"><button className="primary-btn" onClick={() => go('work')}>Explore my work <ArrowUpRight size={18}/></button><button className="ghost-btn" onClick={() => go('contact')}>Start a conversation</button></div><div className="hero-meta"><span><Sparkles size={15}/> AI / LLMs</span><span><Layers3 size={15}/> Full Stack</span><span><ShieldCheck size={15}/> Production-minded</span></div></div>
          <div className="hero-visual"><PortfolioScene/><div className="hero-photo-card"><img src={PROFILE_IMAGE} alt="Nithish Kumar portrait"/><div><span>IDENTITY</span><strong>NITHISH KUMAR</strong></div></div><div className="hero-orbit-note"><span className="pulse-dot"/> INTERACTIVE 3D PROFILE</div></div>
          <button className="scroll-cue" onClick={() => go('about')}><span>SCROLL TO EXPLORE</span><ChevronDown size={18}/></button>
        </section>

        <section id="about" className="section container about-section"><div className="section-head"><span className="section-number">01</span><div><p className="section-kicker">THE ENGINEERING APPROACH</p><h2>From intelligence to <span>impact.</span></h2></div></div><div className="about-grid"><div className="about-statement"><p>I work at the intersection of <strong>AI and product engineering</strong> — designing the model layer, backend, tools, data and interface as one system.</p><p>My strongest work spans conversational AI, Salesforce automation, multi-agent workflows, retrieval systems, computer vision and production web applications.</p><div className="signature-line"><span>Chennai, India</span><span>Available for AI-focused opportunities</span></div></div><div className="capability-list">{capabilities.map(({icon:Icon,title,text})=><article key={title}><Icon size={22}/><div><h3>{title}</h3><p>{text}</p></div><ArrowUpRight size={17}/></article>)}</div></div></section>

        <section id="work" className="section work-section"><div className="container"><div className="section-head"><span className="section-number">02</span><div><p className="section-kicker">SELECTED BUILDS</p><h2>Systems I've <span>shipped.</span></h2></div><p className="section-note">A focused selection of AI, automation, backend and product engineering work.</p></div><div className="work-list">{work.map(item=><article className="work-row" key={item.title}><div className="work-index">{item.index}</div><div className="work-main"><p className="work-category">{item.category}</p><h3>{item.title}</h3><p>{item.description}</p><div className="stack">{item.stack.map(s=><span key={s}>{s}</span>)}</div></div><a className="work-link" href={item.repo} target="_blank" rel="noreferrer" aria-label={`Open ${item.title}`}><Github size={20}/><ArrowUpRight size={17}/></a></article>)}</div></div></section>
