  ...
  try {
    ...
    if (!webhook) {
      return res.status(200).json({
        success: false,
        fallback: true,
        message: 'Contact storage is not configured, so email fallback is being used.'
      });
    }
    ...
    if (!response.ok) return res.status(502).json({ message: 'Message storage is temporarily unavailable.' });
