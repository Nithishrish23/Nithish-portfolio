...
function App() {
  ...
  return <div className="site">
    ...
    <header className="nav-wrap"><nav className="nav container">
      <button className="brand" onClick={() => go('home')} aria-label="Back to home"><span className="brand-mark"><img className="brand-photo" src={PROFILE_IMAGE} alt="Nithish Kumar"/><i/></span><span><strong>Nithish Kumar</strong><small>AI Engineer &amp; Full Stack Developer</small></span></button>
      <div className={`nav-links ${menuOpen ? 'open' : ''}`}>{[['about','about'],['work','Projects'],['experience','Experience'],['contact','Contact']].map(([id,label]) => <button key={id} onClick={() => go(id)}>{label}</button>)}<button onClick={() => go('stack')}>Tech Stack</button><a href="https://github.com/Nithishrish23" target="_blank" rel="noreferrer">GitHub <ArrowUpRight size={14}/></a></div>
